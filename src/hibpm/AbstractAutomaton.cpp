...
StateInterface *ConcreteState::nextState(int symbol) {
  ...
  int targetState =
      this->automaton->automaton->transitionsTo[this->stateNum][symbol];
...
ConcreteState::ConcreteState(ConcreteAutomaton *automaton, int numState) {
  ...
  if (numState > -1) {
    this->isFinalState =
        this->automaton->automaton->areFinalStates[this->stateNum];
  }
}
...
StateInterface *ConcreteAutomaton::nextStates(StateInterface *originState,
                                              int symbol) {
...
int ConcreteAutomaton::getAlphabetSize() { return this->automaton->sigSize; }
...
StateInterface *ComposedAutomaton::nextStates(StateInterface *originState,
                                              int symbol) {
...
ComposedAutomaton::ComposedAutomaton(AbstractAutomaton *automatonRight,
                                     AbstractAutomaton *automatonLeft)
    : initialState(this, automatonRight->getAlphabetSize(),
                   automatonLeft->getInitialState(),
                   automatonRight->getInitialState()) {
...
ComposedState *ComposedAutomaton::insertComposedState(ComposedState *state) {
  ...
  pair<set<pair<list<int>, ComposedState *>, compLists>::iterator, bool> res =
      this->computedSets.insert(p);
...
int ComposedAutomaton::getAlphabetSize() { return this->alphaSize; }
...
StateInterface *ComposedAutomaton::insertState(StateInterface *state) {
  ...
  //
  //        pair<set<pair<list<int>,ComposedState*>>::iterator ,bool> res =
  //        this->computedSets.insert(p);
  //
  //        return res.first->second;
}

ComposedState::ComposedState(ComposedAutomaton *automaton, int alphabetS,
                             StateInterface *stateRight,
                             StateInterface *stateLeft) {
  ...
  this->isFinalState =
      this->stateRight->isFinal() && this->stateLeft->isFinal();
}
...
StateInterface *ComposedState::nextState(int symbol) {
  ...
  if (stLeftRes != nullptr && stRighRes != nullptr) {
    ComposedState *cs = new ComposedState(this->automaton, this->alphabetSize,
                                          stRighRes, stLeftRes);
...
bool ComposedState::isFinal() { return this->isFinalState; }
