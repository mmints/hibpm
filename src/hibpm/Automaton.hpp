...
public:
  ...
  void interComlement(set<int> A, set<int> B, set<int> &complement,
                      set<int> &intersect);
  ...
  list<pair<int, int>> removeUselessStates(list<pair<int, int>> &finalStates,
...
private:
  void reachFinals(Automaton *a1, Automaton *a2, list<pair<int, int>> stacked,
                   list<pair<int, int>> finals);
