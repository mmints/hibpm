//
// Created by Jandson on 20.10.21.
//
...
bool GraphSolver::checkCyclesForwardPos(int ev1, int ev2){
    ...
    if (this->matrix[ev2][ev1].FUTURE_POS){//then there is a cycle forward
        ...
        // COMPUTE CYCLES
        //CYCLES COMPRISES EXATLY ev1, ev2 and all those who insie on ev1 and on ev2
        for (int i = 0; i < this->numEvs; ++i) {
...
void GraphSolver::modifyMatrix(int ev1, int ev2, DIMENSION AXIS, bool nValue){
    ...
    switch (AXIS) {
        ...
        case EXISTENCIAL:
            ...
            if (this->matrix[ev1][ev2].EXISTS_POS && this->matrix[ev2][ev1].EXISTS_NEG){
                this->modifyStatus(ev1,0);
            }else if (this->matrix[ev1][ev2].EXISTS_NEG && this->matrix[ev2][ev1].EXISTS_POS){
                this->modifyStatus(ev2,0);
            }
...
bool GraphSolver::increment(shared_ptr<State> state) {
    ...
    switch (rt) {
        ...
        case RuleType::SUCCESSION:{
            Rule
                rResp = {RuleType::RESPONSE, state->getRule().events},
        ...
        case RuleType::CHAIN_SUCCESSION:{
            ...
            Rule
                rChainResp = {RuleType::CHAIN_RESPONSE, state->getRule().events },
        ...
        case ALTERNATED_SUCCESSION: {
            ...
            Rule
                    rAltResp = {RuleType::ALTERNATED_RESPONSE, state->getRule().events},
