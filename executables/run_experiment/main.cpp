...
ExperimentType selectExperimentType(char *&experiment) {
  if (strcmp(experiment, "--lazy-expand") == 0 ||
      strcmp(experiment, "-le") == 0)
    return LAZY_EXPAND;

  if (strcmp(experiment, "--ego-expand") == 0 || strcmp(experiment, "-ee") == 0)
    return EGO_EXPAND;

  if (strcmp(experiment, "--lazy-expand-shrink") == 0 ||
      strcmp(experiment, "-les") == 0)
...
void printHelp() {
  std::cout
      ...
      "\nOPTIONS:\n"
      << "\t[-h, --help] This message. \n"
      << "\t[-le, --lazy-expand] Perform lazy expand algorithm on given data "
         "set \n"
      << "\t[-ee, --ego-expand] WIP! Perform ego expand algorithm on given "
         "data set \n"
      << "\t[-les, --lazy-expand-shrink] Perform lazy expand shrink algorithm "
         "on given data set \n"
...
std::string createOutName(fs::path &input, fs::path &output,
                          ExperimentType experimentType) {
  std::string out = "";
  switch (experimentType) {
  case LAZY_EXPAND: {
    out =
        output.string() + "/" + input.filename().replace_extension("").string();
    out += "-LAZY-EXPAND";
    break;
  }
  case EGO_EXPAND: {
...
void writeResultsToCSV(const std::string &out, size_t kernelSetSize,
                       size_t solutionSetSize, double parsingTime,
  ...
  // Header
  outfile << "'kernel_set_size';'solution_set_size';'parsing_time';'experiment_"
             "time';'overall_time'"
          << std::endl;
  outfile << kernelSetSize << ";" << solutionSetSize << ";" << parsingTime
          << ";" << experimentTime << ";" << (parsingTime + experimentTime)
          << std::endl;

  outfile.close();
}

void printSets(const std::string &out,
               const RemainderComposition &remainderComposition) {
...
int main(int argc, char **argv) {
  ...
  if (!fs::exists(outputPath)) {
    std::cout << "The output path " << inputFile << " does not exist."
              << std::endl;
    return -1;
  }
  ...
  switch (experimentType) {
  case LAZY_EXPAND: {
    std::cout << "Experiment Running: LAZY_EXPAND"
              << std::endl; // TODO: Add experiment
    ...
    writeResultsToCSV(out, remainderComposition.kernelSet.size(),
                      remainderComposition.solutionSet.size(),
  ...
  case LAZY_EXPAND_SHRINK: {
    std::cout << "TODO: LAZY_EXPAND_SHRINK"
              << std::endl; // TODO: Add experiment
    break;
  }
  }
  ...
  std::cout << "Done.\n Results of the the experiment on model "
            ...
            << " where written to " << outputPath.filename() << std::endl;
  return 0;
}
